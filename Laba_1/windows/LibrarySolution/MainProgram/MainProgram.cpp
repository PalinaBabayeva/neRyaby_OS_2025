...
void testVectorAdvancedOperations() {
    ...
    std::cout << "θ = " << thetaA.getValue() << " рад (" << thetaA.getValue() * 180 / 3.1415 << "°)\n";

    Number rhoB = vecB.getRho();
    Number thetaB = vecB.getTheta();
    std::cout << "vecB: ρ = " << rhoB.getValue() << " (≈5.3852), ";
    std::cout << "θ = " << thetaB.getValue() << " рад (" << thetaB.getValue() * 180 / 3.1415 << "°) - второй квадрант\n";

    Number rhoUnit45 = unitCircle45.getRho();
    Number thetaUnit45 = unitCircle45.getTheta();
    std::cout << "unit45: rho = " << rhoUnit45.getValue() << " (≈1.0000), ";
    std::cout << "θ = " << thetaUnit45.getValue() << " рад (" << thetaUnit45.getValue() * 180 / 3.1415 << "°) - должно быть 45°\n";
    ...
    Vector rotated90 = vec.rotate(Number::create(3.1415 / 2));
    std::cout << "Поворот (1,0) на 90°: (" << rotated90.getX().getValue() << ", " << rotated90.getY().getValue() << ") (ожидается: (0, 1))\n";

    Vector polar = Vector::fromPolar(Number::create(2.0), Number::create(3.1415 / 3));
...
void testPhysicalScenarios() {
    ...
    std::cout << "Результирующая сила: " << totalForce.getRho().getValue() << " Н под углом "
        << totalForce.getTheta().getValue() * 180 / 3.1415 << "°\n";
    ...
    std::cout << "Направление: " << totalDisplacement.getTheta().getValue() * 180 / 3.1415 << "° от севера\n";
